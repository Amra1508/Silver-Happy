...
func Reject_Offer(response http.ResponseWriter, request *http.Request) {
    ...
    query := `UPDATE MESSAGE_PRESTATAIRE SET etat_offre = 'refuse' WHERE id_message = ?`
    _, err := db.DB.Exec(query, idMessage)
    if err != nil {
        http.Error(response, "Erreur lors du refus", http.StatusInternalServerError)
        return
    }

    contenuRefus := "Désolé, je refuse cette offre pour le service demandé."
