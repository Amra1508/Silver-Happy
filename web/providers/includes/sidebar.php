...
<aside class="w-64 bg-[#1C5B8F] text-white min-h-screen flex flex-col shadow-xl font-sans shrink-0 sticky top-0">
    ...
    <nav id="sidebar-nav" class="flex-1 px-4 py-6 space-y-2 overflow-y-auto hidden">
        ...
        <a href="/providers/index.php" class="flex items-center gap-3 px-4 py-3 hover:bg-white/10 hover:text-[#E1AB2B] rounded-xl transition-all">
            Tableau de bord
        </a>

        <a href="/providers/services/events.php" class="flex items-center gap-3 px-4 py-3 text-gray-200 hover:bg-white/10 hover:text-[#E1AB2B] rounded-xl transition-all">
            Mes Evenements
        </a>
        ...
        <a href="/providers/services/historic.php" class="flex items-center gap-3 px-4 py-3 text-gray-200 hover:bg-white/10 hover:text-[#E1AB2B] rounded-xl transition-all">
            Historique
        </a>
        ...
        <a href="/providers/communication/reviews.php" class="flex items-center gap-3 px-4 py-3 text-gray-200 hover:bg-white/10 hover:text-[#E1AB2B] rounded-xl transition-all">
            Mes avis
        </a>

        <a href="/providers/invoice/list_invoice.php" class="flex items-center gap-3 px-4 py-3 text-gray-200 hover:bg-white/10 hover:text-[#E1AB2B] rounded-xl transition-all">
            Facturation
        </a>
    ...
    <div class="p-4 border-t border-white/10 mt-auto">
        <a href="/providers/account/profile.php" class="flex items-center gap-3 px-4 py-3 text-gray-200 hover:bg-white/10 hover:text-[#E1AB2B] rounded-xl transition-all mb-2">
            Mon Profil
        </a>
        <button id="btn-logout-provider" class="w-full flex items-center gap-3 px-4 py-3 text-gray-200 hover:bg-white/10 hover:text-red-700 rounded-xl transition-all">
            Déconnexion
        </button>
    </div>
...
<script src="https://cdn.onesignal.com/sdks/web/v16/OneSignalSDK.page.js" defer></script>
    ...
    <script>
        ...
        function initOneSignalProvider() {
            ...
            OneSignalDeferred.push(async function(OneSignal) {
                ...
                OneSignal.User.PushSubscription.addEventListener("change", subscription => {
                    if (subscription.current.optedIn) {
                        sauvegarderPlayerIDProvider(subscription.current.id);
                    }
                });

                if (OneSignal.User.PushSubscription.optedIn) {
                    sauvegarderPlayerIDProvider(OneSignal.User.PushSubscription.id);
                }
            });
        }

        function sauvegarderPlayerIDProvider(playerID) {
            ...
            .then(data => console.log("ID OneSignal Prestataire synchronisé :", data))
            .catch(error => console.error("Erreur sync OneSignal Prestataire :", error));
        }
    </script>